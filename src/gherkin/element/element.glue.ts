...
export async function whenClick(button: ClickAction, meta: string, index: number, key: string, type: string) {
  ...
  const element = type === ElementType.LINK ?
    await new XPathBuilder().textEquals(MetaAdapter.getLabel(meta, key)).hasExactAttribute(AnchorAttributes.HREF).toWebElement().$(index - 1) :
...
export async function whenDrag(meta: string, sourceIndex: number, sourceKey: string, targetIndex: number, targetKey: string) {
  const sourceSelector = MetaAdapter.getSelector(meta, sourceKey);
  const targetSelector = MetaAdapter.getSelector(meta, targetKey);
...
export async function whenSelectDropdownOption(optIndex: number, context: SelectOptionContext, value: string, meta: string, ddlIndex: number, key: string) {
  const parsed = GherkinAdapter.parseToken(MetaAdapter.getLabel(meta, value));
  const selector = MetaAdapter.getSelector(meta, key);
...
export async function whenSetValue(action: SetValueAction, value: string, meta: string, index: number, key: string) {
  const parsed = GherkinAdapter.parseToken(value);
  const selector = MetaAdapter.getSelector(meta, key);
  const element = await new WebElement(selector).$(index - 1);
...
export async function thenHrefOpensOn(meta: string, index: number, key: string, type: ElementType, not: boolean, target: HrefTargetContext) {
  ...
  const then = type === ElementType.LINK ?
    await new XPathBuilder().textEquals(MetaAdapter.getLabel(meta, key)).hasExactAttribute(AnchorAttributes.HREF).toWebElement().conditions(index - 1) :
  ...
  switch (context) {
    case SelectOptionContext.LABEL: {
      element = await option
        .descendantOrSelf()
        .textEquals(GherkinAdapter.parseToken(MetaAdapter.getLabel(meta, value)))
        .toWebElement()
        .$();
      break;
    }
    case SelectOptionContext.VALUE: {
      element = await option
        .descendantOrSelf()
        .attributeEquals(SelectOptionContext.VALUE, GherkinAdapter.parseToken(MetaAdapter.getLabel(meta, value)))
        .toWebElement()
        .$();
      break;
    }
