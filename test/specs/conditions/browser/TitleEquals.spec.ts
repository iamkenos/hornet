import { TitleEquals } from "@conditions/browser/TitleEquals";
...
describe("@conditions: browser/TitleEquals constructor", () => {
  ...
  it("S01: should set properties upon instantiation", async() => {
    const condition = new TitleEquals(data.any);
...
describe("@conditions: browser/TitleEquals.getResult()", () => {
  afterEach(() => {
    givenJestMocksAreReset();
  });
  ...
  it("S01: should return a passed result", async() => {
    ...
    const condition = new TitleEquals(data.foo);
  ...
  it("S02: should return a passed result if not is true", async() => {
    givenMock(browser.getTitle).mockReturnValue(data.foo);
    const condition = new TitleEquals(undefined, false);
