import { CookieEquals } from "@conditions/browser/CookieEquals";
...
describe("@conditions: browser/CookieEquals constructor", () => {
  ...
  it("S01: should set properties upon instantiation", async() => {
    const condition = new CookieEquals(data.any, data.any);
...
describe("@conditions: browser/CookieEquals.getResult()", () => {
  afterEach(() => {
    givenJestMocksAreReset();
  });
  ...
  it("S01: should return a passed result", async() => {
    ...
    const condition = new CookieEquals(data.foo, data.foo);
  ...
  it("S03: should return a failed result if condition is not met", async() => {
    givenMock(browser.getCookies).mockReturnValue([{ name: data.any, value: data.foo }]);
    const condition = new CookieEquals(data.foo, data.bar);
