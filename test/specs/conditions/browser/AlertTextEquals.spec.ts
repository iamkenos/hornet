import { AlertTextEquals } from "@conditions/browser/AlertTextEquals";
...
describe("@conditions: browser/AlertTextEquals constructor", () => {
  ...
  it("S01: should set properties upon instantiation", async() => {
    const condition = new AlertTextEquals(data.any);
...
describe("@conditions: browser/AlertTextEquals.getResult()", () => {
  afterEach(() => {
    givenJestMocksAreReset();
  });
  ...
  it("S01: should return a passed result", async() => {
    ...
    const condition = new AlertTextEquals(data.foo);
  ...
  it("S02: should return a passed result if not is true", async() => {
    ...
    const condition = new AlertTextEquals(data.foo, false);
